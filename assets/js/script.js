...
var formSubmitHandler = function (event) {
  ...
  var lastAddress = JSON.parse(localStorage.getItem("lastAddress")) || [];

  function saveLastAddress(addressEl){
if (!lastAddress.includes(addressEl)){
  lastAddress.unshift(addressEl);
  if (lastAddress.length> 3){
    lastAddress.pop();
      }
      localStorage.setItem("lastAddress", JSON.stringify(addressEl))
}
  }

function renderLastAddress(){
  var storedAddress = JSON.parse(localStorage.getItem("lastAddress"));
  if (storedAddress !==null){
    var previousContainer = $("#previous-search-container"); 
  previousContainer.innerHTML = "";
    for (var i=0; i<storedAddress.length; i++){
      var button = document.createElement("button");
      $(button).text(storedAddress[i]);
      $(button).addClass("storedButtons");
     previousContainer.append(button);
    }
  } 
  else{
    return;
